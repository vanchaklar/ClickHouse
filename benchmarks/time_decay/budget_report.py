...
latency={r['name']:statistics.median(x['wall_seconds'] for x in m if x['name']==r['name'])*1000 for r in a if any(x['name']==r['name'] for x in m)}
...
for r in sorted(a,key=lambda r:(r['dataset'],r['path'],r['calculation_budget'])):
    if r['name'] not in latency:
        print(f'| {r["dataset"]} | {r["path"]} | {r["calculation_budget"]:g} | NOT TIMED | | | | | |')
        continue
    ms=latency[r['name']]
...
print('Raw-input controls use a 1e-12 relative/absolute comparison tolerance for parallel-merge rounding; their measured deltas are retained in `budget_accuracy.json`.\n')
