...
(
    id UInt8,
    probe Variant(ExponentialTimeDecayingFloat64(10), UInt8),
    key Variant(ExponentialTimeDecayingFloat64(10), UInt8)
)
ENGINE = Memory;
...
SELECT CAST((1., 0., 10.), 'ExponentialTimeDecayingFloat64(10)') IN (SELECT key FROM time_decay_variant_in);
SELECT materialize(CAST((1., 0., 10.), 'ExponentialTimeDecayingFloat64(10)')) IN (SELECT key FROM time_decay_variant_in);
...
SELECT CAST(a, 'Variant(Array(ExponentialTimeDecayingFloat64(10)), UInt8)') IN
    (SELECT CAST(a, 'Variant(Array(ExponentialTimeDecayingFloat64(10)), UInt8)'));

-- Validate the probe and the set independently, including noncanonical ordering fields.
SELECT probe IN (SELECT value FROM VALUES('value Variant(ExponentialTimeDecayingFloat64(10), UInt8)', ((2., 0., 10.))))
