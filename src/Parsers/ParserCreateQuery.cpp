...
Use `workload` in the stored SELECT settings to choose the workload for refresh admission and execution:
...
The stored SELECT's `workload` is used for both refresh admission and execution. DDL workload routing is independent of refresh admission; this setting does not change how CREATE or ALTER statements are scheduled.
