...
When enabled, refreshable materialized views request a query slot before starting refresh execution. The stored SELECT setting `runtime_workload` selects the refresh workload; when empty, the existing `workload` setting is used. This requires a configured QUERY resource for that workload. While queued, the refresh reports `WaitingForResource` in `system.view_refreshes` without occupying a background execution worker.
