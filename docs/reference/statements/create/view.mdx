...
- `SQL SECURITY`: `INVOKER` for normal views and `DEFINER` for materialized views ([configurable by settings](/reference/settings/session-settings/default#default_normal_view_sql_security))
- `DEFINER`: `CURRENT_USER` ([configurable by settings](/reference/settings/session-settings/default#default_view_definer))
...
* `workload` - Workload used by refresh queries. When non-empty, the refresh waits asynchronously
  for a query slot in this workload before execution, without occupying a background schedule pool
  worker. The workload is also used for CPU and I/O resource scheduling. Default: empty.

For example, the following view runs refreshes in the `rmv` workload:

```sql
CREATE MATERIALIZED VIEW rmv
REFRESH EVERY 1 MINUTE SETTINGS workload = 'rmv'
TO target
AS SELECT *
FROM source;
```
