...
/// Diagnostic for the blocked-beneficiary hand-off. Once the admitted allocation can no longer
/// grow, the old eviction policy must evaluate that allocation's request rather than reusing the
/// parked owner's request and evicting the beneficiary itself.
TEST(SchedulerSpaceSharedFixProbe, BlockedBeneficiaryUsesItsOwnRequestForEviction)
{
    SpaceSharedTest t;
    SpaceSharedResourceHolder r(t);
    r.addLimit("/", 10000);
    ...
    heavy.increaseAsync(5000);
    auto small = std::make_unique<ManualAllocation>(queue, "small", 1000, false);
    ASSERT_TRUE(small->waitSyncedFor(std::chrono::seconds(2)));
    ASSERT_EQ(heavy.killCount(), 0u);

    small->increaseAsync(2000);
    heavy.recoveryCheckpoint();

    EXPECT_TRUE(heavy.waitKillsFor(1, std::chrono::seconds(2)))
        << "The beneficiary's blocked request did not evict the old scheduler's victim";
    EXPECT_EQ(small->killCount(), 0u)
        << "Suction reused the parked owner's request and reversed victim selection";
}


/// A beneficiary may recover its own blocked growth by spilling. Completion must be consumed for
/// that allocation even though another allocation owns the older suspended-growth episode.
TEST(SchedulerSpaceSharedFixProbe, BeneficiarySpillCompletionIsConsumed)
{
    SpaceSharedTest t;
    SpaceSharedResourceHolder r(t);
    r.addLimit("/", 10000);
    ...
    heavy.increaseAsync(5000);
    auto small = std::make_unique<ManualAllocation>(queue, "small", 1000, false);
    small->protectAfterPressureRounds(2);
    ASSERT_TRUE(small->waitSyncedFor(std::chrono::seconds(2)));

    small->reconcilePendingIncreaseTo(0);
    small->increaseAsync(2000);
    ASSERT_TRUE(small->waitPressureCountFor(1, std::chrono::seconds(2)))
        << "The blocked beneficiary never received its spill opportunity";
    small->recoveryCheckpoint();

    EXPECT_TRUE(small->waitSyncedFor(std::chrono::seconds(2)))
        << "The beneficiary's spill completion was ignored";
    EXPECT_EQ(small->size(), 1000);
    EXPECT_EQ(heavy.killCount(), 0u);
    EXPECT_EQ(small->killCount(), 0u);
}
...
/// An exhaustive-spill strategy must visit every registered processor before closing the epoch.
/// The current one-claim strategy completes after the first processor and leaves the rest untouched.
TEST(SchedulerSpaceSharedFixProbe, ForcedSpillVisitsEveryRegisteredProcessor)
{
    MemorySpillScheduler scheduler(/*enable_=*/ false);
    ManualSpillProcessor first(4096, /*spill_succeeds_=*/ true);
    ManualSpillProcessor second(2048, /*spill_succeeds_=*/ true);
    ...
    EXPECT_EQ(
        scheduler.getForcedSpillResult(request.epoch).outcome,
        MemorySpillScheduler::ForcedSpillOutcome::Pending)
        << "The epoch closed before every registered processor was visited";
